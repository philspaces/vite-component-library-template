...
const variants = {
  base: "text-white rounded-md transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-indigo-500 disabled:opacity-50 disabled:pointer-events-none",
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ color = "primary", size = "medium", className, ...rest }, ref) => (
    <button
      ref={ref}
      className={twMerge(
        clsx(variants.base, variants[color], variants[size], className),
      )}
      {...rest}
    />
  ),
